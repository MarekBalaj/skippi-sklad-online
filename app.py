...
def manual_prijem():
    ...
    if request.method == 'POST':
        produkt_filter = request.form.get('produkt_filter', '').lower()
        filtered_produkty = [
...
@app.route('/decrement_mnozstvo', methods=['POST'])
def decrement_mnozstvo():
    nazov = request.form['nazov']
    farba = request.form['farba']
    ...
    for idx, produkt in enumerate(produkty):
        if produkt['Názov produktu'] == nazov and produkt['Farba / Dekor'] == farba:
            aktualne_mnozstvo = int(produkt['Množstvo'])
            nove_mnozstvo = aktualne_mnozstvo - 1  # umožní záporné hodnoty
            riadok = idx + 2
            stlpec = sheet.row_values(1).index('Množstvo') + 1
            sheet.update_cell(riadok, stlpec, nove_mnozstvo)
            break
...
@app.route('/increment_mnozstvo', methods=['POST'])
def increment_mnozstvo():
    nazov = request.form['nazov']
    farba = request.form['farba']
    ...
    for idx, produkt in enumerate(produkty):
        if produkt['Názov produktu'] == nazov and produkt['Farba / Dekor'] == farba:
            aktualne_mnozstvo = int(produkt['Množstvo'])
            nove_mnozstvo = aktualne_mnozstvo + 1
            riadok = idx + 2
            stlpec = sheet.row_values(1).index('Množstvo') + 1
            sheet.update_cell(riadok, stlpec, nove_mnozstvo)
            break
